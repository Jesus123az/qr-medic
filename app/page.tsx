    ...
    <div className="overflow-x-hidden">
      ...
      <div className="about-us bg-[#CBE9EF] mx-[3%] flex flex-col items-center">
        <div className="border-t-2 border-[#88CBD7] py-12 w-[95%] flex flex-col items-center">
          <h1 className="text-4xl md:text-6xl font-bold pb-7 text-center">
            About us
          </h1>
          ...
          <div className="about-us-card bg-white rounded-[42px] w-full max-w-[43rem] mt-7 flex flex-col-reverse md:flex-row gap-7 p-6 md:p-9 items-center shadow-2xl">
            <p className="about-us-text text-lg md:text-2xl font-normal">
              ...
              to <span className="font-semibold">empower individuals</span> who
              are unable to help themselves during critical moments. Through
              advanced Augmented Reality technology, we ensure{" "}
              <span className="font-semibold">
                critical health details are instantly available in emergency
                situations.
              </span>{" "}
              Our commitment is to enhance personal security and offer peace of
              mind for individuals and their caregivers when it&apos;s needed
              most.
            </p>
            <Image
              src={oldLady}
      ...
      <div className="how-to-use flex flex-col md:flex-row py-12 md:py-20 gap-8 md:gap-x-11 justify-center items-center">
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-x-12">
          {steps.map(({ number, image, text }) => (
            <div
              key={number}
              className="w-full md:w-52 pl-7 py-3 rounded-3xl bg-white relative"
            >
              ...
              <div
                className={`absolute ${
                  number === 3 ? "-right-12 top-0" : "-right-7 top-7"
                } ${number === 2 && "bg-[#DBEF99] rounded-xl -rotate-6"}`}
              >
                <Image
                  className={number === 2 ? "rotate-6" : ""}
                  src={image}
                  alt="step"
                  width={number === 3 ? 150 : 100}
                  height={100}
                />
              </div>
            </div>
      ...
      <Suspense fallback={<div>Loading....</div>}>
        <div
          ...
          className="services mx-[3%] flex flex-col md:flex-row py-12 md:py-16 justify-center items-center gap-8 md:gap-x-48 bg-[#CBE9EF]"
        >
          ...
          <div className="cards flex flex-col gap-y-10 w-full max-w-[28rem]">
            {[
              {
                icon: process,
                text: "Simple <span class='italic font-semibold'>2 step</span><br />registration process",
              },
              {
                icon: vitals,
                text: "Instant Access to your<br /><span class='italic font-semibold'>Vital Information</span>",
              },
              {
                icon: mobile,
                text: "Efficient information<br /><span class='italic font-semibold'>display in AR</span>",
              },
              {
                icon: aid,
                text: "Access <span class='italic font-semibold'>First aid</span><br />instructional videos",
              },
            ].map((item, index) => (
              <div
                key={index}
                className="w-full relative h-[5.5rem] flex px-6 pt-3 rounded-3xl bg-white"
              >
                <div
                  style={{
                    boxShadow: "1.53px 12.21px 56.93px -2.91px #E4F1BA inset",
                  }}
                  className={`flex justify-center items-center w-28 h-28 absolute z-20 rounded-full bg-white -top-3 ${
                    index % 2 === 1 ? "right-6" : ""
                  }`}
                >
                  <Image
                ...
                <p
                  ...
                  } font-normal text-center text-lg md:text-2xl`}
